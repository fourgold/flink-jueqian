...
public class HotItemApp3 {
    public static void main(String[] args) throws Exception {
        ...
        WindowedStream<UserBehavior, Long, TimeWindow> windowStream = keyedUserBehavior.timeWindow(Time.minutes(10), Time.minutes(5));
        ...
        SingleOutputStreamOperator<Tuple2<Long, Long>> aggregate = windowStream.aggregate(new ItemCountAggFunc3());
...
//        aggregate1.print("b");
    ...
    public static class ItemCountWindowFunc2 implements WindowFunction<Long, ItemCount,Long,TimeWindow> {
        ...
        public void apply(Long aLong, TimeWindow window, Iterable<Long> input, Collector<ItemCount> out) throws Exception {
            System.out.println("当前key值:"+aLong);
    ...
    //第一步聚合,来一个 聚合一个,在一个窗口内,按照key将数据进行统计
    public static class ItemCountAggFunc3 implements AggregateFunction<UserBehavior,Tuple2<Long,Long>,Tuple2<Long,Long>> {
        @Override
        ...
        public Tuple2<Long, Long> add(UserBehavior value, Tuple2<Long, Long> accumulator) {
            accumulator.f0=value.getItemId();
            accumulator.f1=1+accumulator.f1;
            return accumulator;
        }
